...
~~~~~~~~~~~~
    ...
    For details of the data structures that
are returned, see the doc strings for
``load_data`` and ``load_data_wrapper``.
    In practice, ``load_data_wrapper`` is
the function usually called by our neural
network code.
"""
...
def load_data():
    ...
    the validation data, and the test data.'
        ...
        This is numpy ndarray with 50,000 entries.
    ...
    ndarray containing 50,000 entries.
    ...
    each contains only 10,000 images.
        ...
        This is a nice data format, but for use in neural networks
    it's helpful to modify the format of the ``training_data`` a little.
        That's done in the wrapper function ``load_data_wrapper()``, see
    below.
    :return:
    """
    f = gzip.open('../data/mnist_pkl.gz', 'rb')
...
def load_data_wrapper():
    ...
    10,000 2-tuple ``(x, y)``. In each case, ``x`` is a 784-dimensional
    numpy.ndarray containing the input image, and ``y`` is the
    corresponding classification, i.e., the digit values (integers)
    corresponding to ``x``.

        Obviously, this means we're using slightly different formats for
    the training data and the validation / test data. These formats turn
